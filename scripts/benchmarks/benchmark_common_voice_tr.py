...
async def main():
    ...
    parser.add_argument(
        "--manifest",
        type=Path,
        default=Path("tests/transcription/fixtures/expected/common_voice_tr_manifest.json"),
    )
    ...
    for i, sample in enumerate(samples):
        audio_path = Path("tests/transcription/fixtures") / sample["audio_path"]
        ...
        try:
            transcript = await pipeline.process_audio_path(
                audio_path, source="benchmark", language="tr", quality_mode=args.quality
            )

            raw_text = " ".join(s.raw_text for s in transcript.segments)
            cleaned_text = " ".join(s.corrected_text for s in transcript.segments)

            raw_matched, raw_missing, raw_overlap, raw_cer, raw_wer = calculate_overlap_metrics(
                expected, raw_text
            )
            clean_matched, clean_missing, clean_overlap, clean_cer, clean_wer = (
                calculate_overlap_metrics(expected, cleaned_text)
            )
