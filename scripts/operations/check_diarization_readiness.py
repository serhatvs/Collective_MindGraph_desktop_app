...
def main():
    ...
    if os.path.isabs(model_name):
        ...
        print(
            "⚠️ Model name is not an absolute path. Pyannote usually expects a HF repo ID or a local path."
        )
        if not allow_download:
            print(
                "⚠️ Since allow_download is False, Pyannote must find this model in its local cache."
            )
    ...
    try:
        ...
        pipeline = Pipeline.from_pretrained(
            model_name, use_auth_token=os.getenv("CMG_RT_PYANNOTE_TOKEN")
        )
        if pipeline:
