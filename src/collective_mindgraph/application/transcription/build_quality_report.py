...
class BuildTranscriptQualityReport:
    def build_report(
        self,
        ...
        unresolved_segments = sum(
            1 for segment in segments if segment.speaker.startswith("UNRESOLVED")
        )
        overlap_ratio = self._ratio(
            sum(1 for segment in segments if segment.overlap), segment_count
        )
    ...
    def _comparison(
        ...
        transcript_tokens = Counter(
            _TOKEN_RE.findall(" ".join(item.corrected_text for item in transcript.segments).lower())
        )
        reference_tokens = Counter(
            _TOKEN_RE.findall(" ".join(item.corrected_text for item in reference.segments).lower())
        )
        ...
        speaker_match_ratio = BuildTranscriptQualityReport._ratio(speaker_matches, comparable)
