...
from .edit_alignment import MetricResult as MetricResult
from .edit_alignment import align as _align
from .edit_alignment import (
    edit_distance_with_operations as edit_distance_with_operations,
)
from .edit_alignment import metric_result as _metric_result
...
def normalize_text(text: str, policy: NormalizationPolicy | None = None) -> str:
    ...
    if selected.normalize_apostrophes:
        apostrophe_translation: dict[int, str | int | None] = {
            ord(char): "'" for char in _APOSTROPHES
        }
        value = value.translate(str.maketrans(apostrophe_translation))
    if selected.lowercase:
        value = _language_aware_lower(value, selected.language)
    if selected.normalize_numbers:
        value = _INTEGER_RE.sub(
            lambda match: _normalize_integer(match.group(0), selected.language), value
        )
    ...
    *,
    ...
    for display_term in terms:
        ...
        for start_index in occurrences:
            ...
            mapped_indices = [index for index in mapped if index is not None]
            is_contiguous = (
                len(mapped_indices) == len(mapped)
                and bool(mapped_indices)
                and mapped_indices
                == list(
                    range(
                        mapped_indices[0],
                        mapped_indices[0] + len(mapped_indices),
                    )
                )
            )
            recognized = bool(
                is_contiguous
                and [hypothesis_tokens[index] for index in mapped_indices] == term_tokens
            )
            if recognized:
                status = "correct"
...
def _language_aware_lower(text: str, language: str) -> str:
    if language.casefold().startswith("tr"):
        turkish_lower_translation: dict[int, str | int | None] = {
            ord("I"): "ı",
            ord("İ"): "i",
        }
        return text.translate(str.maketrans(turkish_lower_translation)).lower()
