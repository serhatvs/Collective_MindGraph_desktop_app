...
_ACTION_PATTERNS = (
    ...
    re.compile(
        r"(.+\s+(?:yapmalıyız|etmeliyiz|yapmalısın|etmelisin|iyileştirmeliyiz|yapmamız gerekiyor|etmemiz gerekiyor|gerekiyor|lazım|gerek|gerekmektedir))\b",
        re.IGNORECASE,
    ),
    # 2. Future Action
    re.compile(
        r"(.+\s+(?:yapacağız|edeceğiz|yapılacak|edilecek|bakılacak|kontrol edilecek|test edilecek|göstereceğiz))\b",
        re.IGNORECASE,
    ),
    # 3. Request/Directive
...
_DECISION_PATTERNS = (
    ...
    re.compile(
        r"(.+\s+(?:kararlaştırıldı|karar verildi|karar verdik|anlaşıldı|kararı alındı))\b",
        re.IGNORECASE,
    ),
...
class ExtractTranscriptInsights:
    def build_summary(
        self,
        ...
        transcript.people = sorted(
            {
                ...
                if s.speaker
                and "Speaker_" not in s.speaker
                and "UNRESOLVED_" not in s.speaker
    ...
    def _action_items(self, segments: list[TranscriptSegment]) -> list[TaskItem]:
        ...
        for segment in segments:
            ...
            for sentence in sentences:
                ...
                if (
                    not raw_speaker
                    or "Speaker_" in raw_speaker
                    or "UNRESOLVED_" in raw_speaker
                    or raw_speaker == "Unknown"
                ):
