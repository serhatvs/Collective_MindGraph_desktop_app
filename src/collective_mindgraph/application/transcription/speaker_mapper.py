...
from collective_mindgraph.application.transcription.time_ranges import (
    overlap_duration,
    overlap_ratio,
)
...
class StableSpeakerMapper:
    ...
    def _score_candidate(
        ...
        score = 0.0
        candidate_segments = [
            segment for segment in prior_segments if segment.speaker == stable_speaker
        ]
        for turn in turns:
...
def _group_turns_by_label(
    diarization_turns: list[DiarizationTurn],
) -> dict[str, list[DiarizationTurn]]:
