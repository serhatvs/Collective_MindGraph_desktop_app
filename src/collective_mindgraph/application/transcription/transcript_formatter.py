...
from collective_mindgraph.application.transcription.time_ranges import format_timestamp
...
def format_transcript(transcript: ConversationTranscript, *, corrected: bool = True) -> str:
    return "\n".join(
        format_segment(segment, corrected=corrected) for segment in transcript.segments
    )
...
def build_speaker_stats(transcript: ConversationTranscript) -> list[SpeakerStats]:
    ...
    for segment in transcript.segments:
        ...
        item.speaking_seconds = round(
            item.speaking_seconds + max(0.0, segment.end - segment.start), 3
        )
...
def build_file_transcription_response(
