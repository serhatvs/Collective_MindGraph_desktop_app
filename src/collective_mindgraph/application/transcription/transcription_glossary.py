...
GLOBAL_GLOSSARY_PATH = (
    Path(__file__).resolve().parents[2] / "config" / "transcription_glossary.tr.json"
)
...
def resolve_transcription_glossary(
    ...
    *,
    ...
    max_prompt_chars = max(
        0, int(getattr(settings, "transcription_glossary_max_prompt_chars", 1500))
    )
    ...
    omitted_count = (
        omitted_empty + omitted_too_long + omitted_by_term_limit + omitted_by_prompt_limit
    )
    metadata = {
