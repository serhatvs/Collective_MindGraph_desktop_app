...
class AudioCaptureController(QObject):
    ...
    def _handle_recorder_state_changed(self, state: QMediaRecorder.RecorderState) -> None:
        if (
            state == QMediaRecorder.RecorderState.RecordingState
            and self._pending_output_path is not None
        ):
...
def _read_incremental_pcm_level(
    path: Path, byte_offset: int, header_size: int = 44
...
def list_audio_inputs() -> tuple[AudioInputDeviceInfo, ...]:
