...
from collective_mindgraph.application.transcription.conversation_ids import new_segment_id
from collective_mindgraph.application.transcription.speaker_mapper import (
    StableSpeakerMapper,
)
from collective_mindgraph.application.transcription.time_ranges import overlap_duration
...
def _align_asr_segment(
    ...
    for word in asr_segment.words[1:]:
        ...
        if raw_speaker == current_raw_speaker:
            ...
            continue
        grouped.append(
            _chunk_from_words(asr_segment, current_words, current_raw_speaker, diarization_turns)
        )
        current_words = [word]
...
def _all_words_timed(asr_segment: ASRSegment) -> bool:
    return all(
        word.start is not None and word.end is not None and float(word.end) > float(word.start)
...
def _best_speaker_turn(
    start: float, end: float, turns: list[DiarizationTurn]
...
def _has_overlap(
    start: float, end: float, turns: list[DiarizationTurn], primary_label: str
) -> bool:
    active = [
        turn
        for turn in turns
