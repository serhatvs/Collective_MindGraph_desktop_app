...
This module is intentionally independent from ``TranscriptionConfiguration`` so scripts and the
...
def resolve_asr_runtime_config(environ: Mapping[str, str] | None = None) -> ASRRuntimeConfig:
    ...
    runtime_profile = (
        _env_first(env, ("CMG_RUNTIME_PROFILE", "CMG_RT_RUNTIME_PROFILE"), "cpu").strip().lower()
    )
    ...
    return ASRRuntimeConfig(
        ...
        asr_device=_env_first(
            env, logical_envs["asr_device"], profile_defaults["asr_device"]
        ).strip()
        ...
        asr_language=_optional_env_first(
            env, logical_envs["asr_language"], profile_defaults["asr_language"]
        ),
...
def probe_torch_cuda() -> TorchCudaProbe:
    ...
    try:
        ...
        device_count = (
            int(torch.cuda.device_count()) if hasattr(torch.cuda, "device_count") else None
        )
        return TorchCudaProbe(
            available=available,
...
def add_cuda_dll_directories() -> list[str]:
    ...
    if added:
        ...
        if missing:
            os.environ["PATH"] = (
                os.pathsep.join([*missing, current_path])
                if current_path
                else os.pathsep.join(missing)
            )
    return added
...
def build_asr_diagnostics(
    settings: Any, asr_provider: Any, *, llm_provider: Any | None = None
) -> dict[str, Any]:
    cuda_probe = probe_torch_cuda()
    local_llm_enabled = str(getattr(settings, "llm_provider", "")).lower() not in {
        "",
        "none",
        "disabled",
    }
    return {
        ...
        "LLM provider resolved": getattr(llm_provider, "provider_name", None)
...
def _optional_env_first(
    env: Mapping[str, str], names: tuple[str, ...], default: str | None
) -> str | None:
...
def _collect_overrides(
    env: Mapping[str, str], logical_envs: Mapping[str, tuple[str, ...]]
) -> dict[str, str]:
