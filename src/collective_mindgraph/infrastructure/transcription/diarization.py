...
from collective_mindgraph.application.transcription.contracts import DiarizationTurn, SpeechRegion
...
from collective_mindgraph.infrastructure.security.local_resources import validate_local_model_path
...
def build_diarizer(settings: TranscriptionConfiguration) -> BaseDiarizer:
    ...
    try:
        ...
        LOGGER.warning(
            "Falling back to SingleSpeakerFallbackDiarizer because pyannote failed: %s", exc
        )
        return SingleSpeakerFallbackDiarizer()
...
def _looks_duplicate(left: DiarizationTurn, right: DiarizationTurn) -> bool:
    return (
        left.speaker == right.speaker
        and overlap_ratio(left.start, left.end, right.start, right.end) >= 0.85
    )
