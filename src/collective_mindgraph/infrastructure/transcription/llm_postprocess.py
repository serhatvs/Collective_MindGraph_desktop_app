...
from collective_mindgraph.infrastructure.security.local_resources import validate_local_endpoint
...
class LLMPostProcessor:
    def __init__(
        self, provider: BaseLLMProvider, batch_size: int, context_segments: int = 0
    ) -> None:
    ...
    async def apply(
        self, conversation_id: str, language: str | None, segments: list[TranscriptSegment]
...
def build_llm_postprocessor(settings: TranscriptionConfiguration) -> LLMPostProcessor:
...
def _parse_json_results(
    content: str, source_segments: list[TranscriptSegment]
