...
from collective_mindgraph.application.transcription.contracts import ASRSegment
...
from .numeric_values import optional_float as _optional_float
        ...
        *,
        ...
        audit: dict[str, object] = {
            ...
            "triggers": [
                {
                    **trigger.to_metadata(),
                    "segment_start": first_pass_segments[trigger.segment_index].start
                    + absolute_offset,
        ...
        for region in regions:
            ...
            if selection.selected_pass == "second" and second_pass_segments:
                selected = [
                    _with_selective_metadata(segment, selection_metadata)
                    for segment in second_pass_segments
                ]
                replacements[min(region.segment_indices)] = (region_indices, selected)
                audit["number_of_replaced_segments"] = int(
                    audit["number_of_replaced_segments"]
                ) + len(region.segment_indices)
            else:
                retained = [
        ...
        if planned and _regions_can_merge(
            ...
            planned[-1] = RetranscriptionRegion(
                ...
                segment_indices=tuple(
                    dict.fromkeys([*previous.segment_indices, *candidate.segment_indices])
                ),
                trigger_reasons=tuple(
                    dict.fromkeys([*previous.trigger_reasons, *candidate.trigger_reasons])
                ),
            )
        else:
...
def _with_selective_metadata(
    segment: ASRSegment, selection_metadata: dict[str, object]
) -> ASRSegment:
...
def _absolute_region_metadata(
    region: RetranscriptionRegion, absolute_offset: float
) -> dict[str, object]:
