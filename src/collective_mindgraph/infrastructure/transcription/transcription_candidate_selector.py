...
def _glossary_preservation_penalty(
    first_text: str, second_text: str, terms: Iterable[str]
) -> float:
    ...
    lost = [
        term
        for term in terms
        if term.casefold() in first_folded and term.casefold() not in second_folded
    ]
