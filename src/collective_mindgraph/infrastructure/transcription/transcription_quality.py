...
def estimate_transcription_confidence(
    *,
    ...
    audio_label = (
        str(audio_quality.get("audio_quality_label"))
        if audio_quality and audio_quality.get("audio_quality_label")
        else None
    )
    ...
    turkish_score = (
        turkish_text_sanity_score(transcript_text) if (language or "").lower() == "tr" else None
    )
