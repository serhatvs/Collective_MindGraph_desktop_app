...
class EnergyVAD(BaseVAD):
    ...
    def __init__(self, settings: TranscriptionConfiguration) -> None:
        ...
        for frame_index, active in enumerate(speech_mask):
            ...
            if frame_index - active_start >= min_speech_frames:
                start = max(0.0, active_start * self._frame_ms / 1000 - padding_seconds)
                    ...
                    (frame_index - silence_run + 1) * self._frame_ms / 1000 + padding_seconds,
                )
                regions.append(
                    SpeechRegion(
                        start=start,
            ...
            regions.append(
                SpeechRegion(
                    ...
                    confidence=_region_confidence(
                        smoothed_energies, active_start, len(smoothed_energies)
                    ),
                )
            )
...
def _install_torchaudio_compatibility() -> None:
    """Install the legacy backend probe only when Silero is requested."""
...
def build_vad(settings: TranscriptionConfiguration) -> BaseVAD:
...
def _frame_energies(samples: np.ndarray, sample_rate: int, frame_ms: int) -> np.ndarray:
    ...
    return np.array(
        [float(np.sqrt(np.mean(np.square(frame)))) for frame in frames], dtype=np.float32
    )
...
def _merge_regions(
    regions: list[SpeechRegion], merge_gap_seconds: float = 0.05
) -> list[SpeechRegion]:
    if not regions:
        return []
...
def _split_long_regions(
    ...
    target_seconds = (
        target_region_seconds
        if 0 < target_region_seconds < max_region_seconds
        else max_region_seconds / 2
    )
    ...
    for region in regions:
        ...
        end_frame = min(
            len(frame_energies), max(start_frame + 1, int(np.ceil(region.end / frame_seconds)))
        )
        ...
        while (end_frame - current_start) > max_frames:
            ...
            split_frame = _best_split_frame(
                frame_energies, search_start, search_end, fallback=desired_split
            )
