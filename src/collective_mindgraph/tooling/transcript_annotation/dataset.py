...
from collective_mindgraph.application.transcription.evaluation import NormalizationPolicy
...
from .dataset_manifest import annotation_statistics as _annotation_statistics
from .dataset_manifest import migrate_manifest as _migrate_manifest
from .dataset_manifest import utc_now as _utc_now
from .dataset_manifest import validate_manifest as _validate_manifest
from .dataset_manifest import (
    validate_unique_segment_ids as _validate_unique_segment_ids,
)
from .dataset_transcript_mapping import model_payload as _model_payload
from .dataset_transcript_mapping import (
    segments_from_transcript as _segments_from_transcript,
)
from .file_hashing import sha256_file as sha256_file
...
class AnnotationDataset:
    ...
    def boundary_warnings(
        ...
        *,
        ...
        index = next(
            index for index, item in enumerate(ordered) if item["segment_id"] == segment_id
        )
        warnings: list[str] = []
        ...
        for recording in self.recordings:
            ...
            for segment in recording.get("segments", []):
                ...
                if (
                    start < 0.0
                    or end <= start
                    or (recording.get("duration") and end > float(recording["duration"]))
                ):
                    errors.append(f"invalid boundaries for {recording_id}/{segment_id}")
                if (
                    segment.get("annotation_status") == "reviewed"
                    and not str(segment.get("reference_text") or "").strip()
                ):
