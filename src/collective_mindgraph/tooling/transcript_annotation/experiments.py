...
from .experiment_analysis import (
    filter_results_for_plan as filter_results_for_plan,
)
from .experiment_output import (
    write_experiment_outputs as write_experiment_outputs,
)
from .experiment_runtime import current_git_commit, environment_information
...
def reviewed_reference_text(recording: dict[str, Any]) -> str | None:
    lines = [
        ...
        for segment in sorted(
            recording.get("segments", []), key=lambda item: float(item["reviewed_start"])
        )
...
def hypothesis_for_reviewed_regions(recording: dict[str, Any], transcript: Any) -> tuple[str, str]:
    payload = (
        transcript.model_dump(mode="json") if hasattr(transcript, "model_dump") else transcript
    )
    reviewed_intervals = [
    ...
    for segment in payload.get("segments", []):
        ...
        if reviewed_intervals and not any(
            max(start, left) < min(end, right) for left, right in reviewed_intervals
        ):
            continue
    ...
    *,
    ...
    try:
        ...
        if reference:
            ...
            domain = evaluate_domain_terms(
                reference, cleaned_text or raw_text, glossary_terms, policy=policy
            )
...
def build_experiment_report(
    ...
    *,
    ...
    expected_experiment_ids = (
        experiment_plan_ids(configurations, planned_ids) if planned_ids is not None else None
    )
    ...
    if condition_counts:
        lines.extend(
            f"- `{condition}`: {count}" for condition, count in sorted(condition_counts.items())
        )
    else:
    ...
    for configuration in configurations:
        lines.append(
            f"- `{configuration['mode']}`: `{json.dumps(configuration, ensure_ascii=False, sort_keys=True)}`"
        )
    ...
    if best:
        ...
        lines.append(
            "No best configuration is declared because no valid human-reference metrics were available."
        )
    ...
    if regressions:
        ...
        lines.append(
            "No condition-level regressions can be calculated without tagged, reviewed references."
        )
    ...
    for item in failures:
        lines.append(
            f"  - `{item.get('recording_id')}` / `{item.get('configuration', {}).get('mode')}`: {item['error']}"
        )
    lines.append(
        f"- Excluded recordings: `{', '.join(excluded_recordings) if excluded_recordings else 'none'}`"
    )
