...
def test_session_detail_panel_orders_main_children_before_side_children():
    app = QApplication.instance() or QApplication([])
    detail = SessionDetail(
        session=Session(
            id=1,
    ...
    assert [root_item.child(index).text(0) for index in range(root_item.childCount())] == [
        "Main branch",
        "Side branch 1",
        "Side branch 2",
    ]
...
def test_session_detail_panel_orders_root_nodes_by_id():
    app = QApplication.instance() or QApplication([])
    detail = SessionDetail(
        session=Session(
            id=1,
...
def test_session_detail_panel_orders_equal_slot_side_children_by_id():
    app = QApplication.instance() or QApplication([])
    detail = SessionDetail(
        session=Session(
            id=1,
...
def test_session_detail_panel_orders_orphan_bucket_children_like_graph_children():
    app = QApplication.instance() or QApplication([])
    detail = SessionDetail(
        session=Session(
            id=1,
    ...
    assert orphan_bucket.text(0) == "Unlinked"
