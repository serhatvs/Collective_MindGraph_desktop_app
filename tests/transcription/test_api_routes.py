...
from collective_mindgraph.application.transcription.contracts import (  # noqa: E402
...
from collective_mindgraph.engine.api.routes import router  # noqa: E402
...
def build_client(
    ...
    *,
    ...
    llm_provider: StubProviderInfo | None = None,
    transcribe_error: Exception | None = None,
) -> tuple[TestClient, StubTranscribeRecording, StubQualityService]:
    app = FastAPI()
    transcription_service = StubTranscribeRecording(
        transcript,
    ...
    setting_values = {
        "app_name": "Collective MindGraph Engine",
        "vad_provider": "silero",
        "asr_provider": "auto",
    ...
    if settings is not None:
        setting_values.update(vars(settings))
    resolved_settings = types.SimpleNamespace(**setting_values)
    app.state.engine_context = types.SimpleNamespace(
        settings=resolved_settings,
        transcribe_recording=transcription_service,
        build_quality_report=quality_service,
    )
...
def test_transcribe_file_route_cleans_up_temp_file_when_transcription_raises(tmp_path):
    ...
    assert request["language"] == "en"
    assert request["source"] == "upload"
    assert request["source_path"].parent == tmp_path
    assert request["source_path"].name.startswith("upload_seg_")
    assert request["source_path"].suffix == ".flac"
    assert not Path(request["source_path"]).exists()
