...
from collective_mindgraph.application.transcription.contracts import ASRSegment, SpeechRegion
from collective_mindgraph.engine.settings import EngineSettings
from collective_mindgraph.infrastructure.transcription import asr as asr_module
...
def test_extract_wav_region_preserves_expected_duration(tmp_path: Path):
    ...
    region_path = _extract_wav_region(
        source, start_seconds=0.5, end_seconds=1.75, target_dir=tmp_path
    )
    try:
...
def test_build_asr_auto_prefers_local(monkeypatch):
    ...
    provider = asr_module.build_asr(EngineSettings())
...
def test_build_asr_gpu_required_raises_when_cuda_unavailable(monkeypatch):
    ...
    settings = EngineSettings(asr_device="cuda", gpu_required=True)
...
def test_build_asr_cuda_request_reports_cpu_fallback_when_gpu_not_required(monkeypatch):
    ...
    provider = asr_module.build_asr(EngineSettings(asr_device="cuda", gpu_required=False))
...
def test_explicit_mock_asr_status_is_not_fallback():
    provider = asr_module.build_asr(EngineSettings(asr_provider="mock"))
