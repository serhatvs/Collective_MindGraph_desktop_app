...
def get_expected_terms():
    expected_path = Path(
        "tests/transcription/fixtures/expected/turkish_meeting_sample.expected.txt"
    )
...
async def test_asr_quality_smoke_turkish():
    audio_path = Path("tests/transcription/fixtures/audio/turkish_meeting_sample.wav")

    if not audio_path.exists():
        pytest.skip(
            "Turkish audio fixture missing. Run: "
            "python scripts/datasets/prepare_turkish_audio_fixture.py"
        )
    ...
    transcript = await pipeline.process_audio_path(
        audio_path, source="smoke_test", language="tr", quality_mode="accurate"
    )
