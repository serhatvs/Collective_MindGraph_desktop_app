...
from collective_mindgraph.application.transcription.contracts import SpeechRegion, TranscriptSegment
from collective_mindgraph.engine.settings import EngineSettings
from collective_mindgraph.infrastructure.audio.wav_files import create_temporary_wav_path
from collective_mindgraph.infrastructure.transcription import (
    recording_processing_helpers as processing_helpers,
)
from collective_mindgraph.infrastructure.transcription import (
    recording_processor as orchestrator_module,
)
from collective_mindgraph.infrastructure.transcription.llm_postprocess import (
    LLMPostProcessor,
    MockLLMProvider,
)
...
def test_runtime_status_exposes_immutable_provider_snapshot(tmp_path, monkeypatch):
    ...
    settings = EngineSettings(data_dir=tmp_path / "data", temp_dir=tmp_path / "temp")
...
async def test_region_extraction_cancellation_cleans_completed_temp(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(processing_helpers, "extract_wav_region", blocking_extract)
    task = asyncio.create_task(
        processing_helpers.extract_wav_region_owned(
            tmp_path / "source.wav",
            0.0,
            1.0,
            tmp_path,
...
async def test_cancellation_safe_thread_waits_for_file_consumer(tmp_path):
    ...
    async def run_owner() -> None:
        try:
            await processing_helpers.to_thread_cancellation_safe(
                blocking_consumer,
                owned_path,
            )
        finally:
