...
async def test_project_turkish_meeting_asr_quality():
    if not AUDIO_PATH.exists():
        pytest.skip(
            ...
            "To run this validation later, record tests/transcription/fixtures/audio/turkish_meeting_sample.wav "
    ...
    transcript = await pipeline.process_audio_path(
        AUDIO_PATH, source="meeting_test", language="tr", quality_mode="accurate"
    )
    ...
    assert "ı" in cleaned_text or "ş" in cleaned_text or "ğ" in cleaned_text, (
        "Turkish characters lost"
    )
