...
from collective_mindgraph.application.transcription.turkish_cleanup import load_glossary
...
from collective_mindgraph.infrastructure.transcription.selective_retranscription import (
    SegmentRetranscriptionTrigger,
    build_retranscription_regions,
    find_retranscription_triggers,
)
from collective_mindgraph.infrastructure.transcription.transcription_candidate_selector import (
...
def test_glossary_compatible_asr_call_preserves_regions_and_supports_legacy_provider(
    tmp_path: Path,
):
    glossary = resolve_transcription_glossary(EngineSettings(), session_terms=["MindGraph"])
    ...
    class GlossaryProvider:
        ...
        def transcribe(
            self, audio_path, language, received_regions, quality_mode, *, glossary=None
        ):
...
def test_candidate_selector_requires_minimum_improvement_and_retains_worse_second_pass():
    ...
    marginal = _segment(
        0.0, 2.0, "toplanti notlari hazir", avg_logprob=-0.22, word_probability=0.91
    )
...
def test_glossary_precedence_deduplication_and_limits(tmp_path: Path):
    ...
    project_path.write_text(
        '["ProjectTerm", "FastAPI", "VeryLongProjectTerminology"]', encoding="utf-8"
    )
    settings = EngineSettings(
...
def test_glossary_file_loader_flattens_categories_and_reports_invalid_json(tmp_path: Path):
    ...
    glossary_path.write_text(
        '{"project": ["MindGraph"], "technical": ["FastAPI"]}', encoding="utf-8"
    )
...
def test_faster_whisper_hotwords_are_forwarded_when_supported(tmp_path: Path):
    ...
    _segments, _info, call_metadata = _call_faster_whisper_transcribe(
        ...
        language="tr",
        profile=resolve_asr_quality_profile(EngineSettings(), "balanced"),
...
def test_old_diagnostics_payload_receives_backward_compatible_defaults():
    diagnostics = TranscriptionDiagnostics(
        provider="faster_whisper", model="small", audio_duration=1.0
    )
...
async def test_pipeline_selectively_replaces_only_weak_segment_and_preserves_raw_candidates(
    ...
    monkeypatch.setattr(
        "collective_mindgraph.infrastructure.transcription.recording_processor.normalize_audio",
    ...
    assert (
        metadata["regions"][0]["second_pass_raw_segments"][0]["text"] == "teknik toplanti duzeldi"
    )
    assert asr.quality_modes == ["max_quality", "selective_recovery"]


@pytest.mark.asyncio
async def test_pipeline_retains_first_pass_when_stronger_model_is_unavailable(
    ...
    pipeline = RecordingProcessor(settings, vad=_NoVAD(), asr=asr, diarizer=_NoDiarizer())

    transcript = await pipeline.process_audio_path(
        audio_path, source="test", language="tr", include_summary=False
    )
