...
from collective_mindgraph.application.transcription.live_capture import (
    CaptureLiveAudio,
    LiveCaptureSession,
)
...
def test_replace_tail_drops_segments_that_cross_reprocess_boundary():
    ...
    replaced = CaptureLiveAudio._replace_tail(existing, incoming, from_second=3.0)

    assert [segment.segment_id for segment in replaced] == ["seg_1", "seg_3"]


def test_window_start_uses_bounded_recent_history():
    service = object.__new__(CaptureLiveAudio)
    service._settings = EngineSettings(
        stream_partial_window_seconds=8.0,
        stream_overlap_seconds=1.5,
    )
    session = LiveCaptureSession(conversation_id="conv_1", language="en")
...
def test_compact_buffer_keeps_recent_audio_only():
    ...
    session = LiveCaptureSession(conversation_id="conv_2", language="en")
    bytes_per_second = (
        service._settings.sample_rate
        * service._settings.channels
        * service._settings.sample_width_bytes
    )
