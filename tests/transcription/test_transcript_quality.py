...
@pytest.mark.asyncio
@patch("collective_mindgraph.infrastructure.transcription.recording_processor.normalize_audio")
@patch("collective_mindgraph.infrastructure.transcription.recording_processor.wav_duration_seconds")
@patch(
    "collective_mindgraph.infrastructure.transcription.recording_processor._build_processing_windows"
)
async def test_orchestrator_calls_normalization(
    ...
    settings = EngineSettings(temp_dir=tmp_path)
    vad = MagicMock()
    ...
    pipeline = RecordingProcessor(settings, vad=vad, asr=asr, diarizer=diarizer)
