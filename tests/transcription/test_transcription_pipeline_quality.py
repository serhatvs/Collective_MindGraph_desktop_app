...
from collective_mindgraph.application.transcription.contracts import ASRSegment, TranscriptSegment
from collective_mindgraph.application.transcription.speaker_mapper import StableSpeakerMapper
...
from collective_mindgraph.infrastructure.transcription.alignment import merge_transcript_segments
...
from collective_mindgraph.infrastructure.transcription.transcription_quality import (
    estimate_transcription_confidence,
)
...
def _write_sine_wav(
    path: Path, *, amplitude: float, sample_rate: int = 16000, duration: float = 1.0
) -> None:
    frames = bytearray()
