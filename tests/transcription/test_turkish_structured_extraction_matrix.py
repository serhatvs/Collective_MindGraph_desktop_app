...
def _run_extraction(text, speaker="Speaker_1"):
    segments = [
        TranscriptSegment(
            segment_id="s1", start=0.0, end=5.0, speaker=speaker, raw_text=text, corrected_text=text
        )
    ]
    transcript = ConversationTranscript(
        conversation_id="test_matrix", source="test", segments=segments, language="tr"
    )
    service = ExtractTranscriptInsights()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Konuşmacı ayrımı için diarization tarafını iyileştirmeliyiz.",
            "diarization tarafını iyileştirmeliyiz",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "Raw transcript ve cleaned transcript ayrı tutulacak.",
            "Raw transcript ve cleaned transcript",
        ),
        ("Bu şekilde ilerlemeye karar verdik.", "Bu şekilde ilerlemeye"),
        (
            "Toplantı sonunda VAD varsayılanlarını değiştirme kararı alındı.",
            "Toplantı sonunda VAD varsayılanlarını değiştirme",
        ),
        ("FastAPI endpointi bu sprintte kalacak.", "FastAPI endpointi bu sprintte"),
        (
            "Cleaned transcript özetleme için varsayılan kaynak olacak.",
            "Cleaned transcript özetleme için varsayılan kaynak",
        ),
    ],
)
...
def test_turkish_extraction_false_positives(negative_text):
    ...
    filtered_actions = [
        item for item in action_items if "default fallback" not in (item.confidence_note or "")
    ]
